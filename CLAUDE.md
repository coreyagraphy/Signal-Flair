...
**Case Zero provenance — RULED by Corey 2026-08-19 ("18 was the score… I grant you autonomy"), closing the three open questions from the provenance trace:**
1. **The 18 STANDS** as the operator-recorded baseline. The June worksheet rows beside it do not average to 18 and the site now says so plainly (the "Provenance, on the record" proof-note) instead of retrofitting them. Never delete the note, never adjust the rows to force reconciliation.
2. **91 is a dated historical reading (Aug 3), no longer "current."** The current reading is measured, not asserted: **98/100 on 2026-08-19** by the live public scanner (Access 100 · Structure 100 · Trust 100 · Answers 90, weighted .30/.30/.20/.20 → 98; three consecutive samples, identical). Trajectory is now **18 → 73 → 91 → 98**, and the current reading's model is the four-signal Signal Pulse — labelled as such everywhere, never conflated with the six-layer plain-average method. Future current readings should follow this pattern: measured by the public scanner, derivation published, reproducible at /pulse.
3. **June 6 stands** as the published baseline date (six contemporaneous commits; proof.json has carried it since inception). The Proof OS repo's 2026-06-02 is the divergent record.
**Mental Vision card (#proof, Corey-requested 2026-08-19):** the parent studio measured by the same instrument — 92/100 on 2026-08-19 (Access 100 · Structure 100 · Trust 100 · Answers 60). The Answers 60 is printed on the card deliberately; do not remove the imperfect bucket — showing it IS the brand. Re-date the card only with a fresh measured reading. The wordmark is MV's own hero voice, measured live: Unbounded 900, uppercase, -0.03em, cycling the FULL MV accent rotation teal `#2dd4bf` → violet `#a78bfa` → pink `#f472b6` → amber `#fbbf24`. **The violet/pink in that cycle are a Corey-authorised exception (2026-08-19) to the no-violet rule — verbatim quotation of MV's brand, valid ONLY inside `.mvp-wordmark`.** The no-violet rule still holds everywhere else on the site. Unbounded is MV's face, never Signal Flair's.
