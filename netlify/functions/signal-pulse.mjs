 ...
 * deterministic checks on the raw HTML. The full Signal Score™ (The Breakdown — live
 * AI-visibility tests where supported, human verification) is the slower follow-up — it is
 * NOT computed here, so nothing is overstated. Lead delivery is Netlify Forms (client-side
 * mirror); GHL was retired 2026-08-18.
 */
...
export const handler = async (event) => {
  ...
  // Lead delivery happens client-side via the Netlify Forms mirror (email to outreach@).
  // GHL was retired 2026-08-18. BOS will connect to this flow when it lands — the JSON
  // result below is the contract a future intake can consume.
