...
- **mentalvision.ai apex property VERIFIED.** Next.js App Router 404s bare `.html` paths in public/, so added `public/gsv.txt` + a forced-200 rewrite in `netlify.toml` (`/google5a5fea2e86d232cd.html` → `/gsv.txt`); redeployed; confirmed live 200. Once the file served, GSC auto-verified the apex property ("Ownership auto verified", HTML file method — Corey confirmed the success screen). Sitemap submission for MV would not register through browser automation (GSC controlled input) and screenshots hang on this pane, so MV sitemap is left to submit manually (20 sec) — or Google auto-discovers it since `mentalvision.ai/robots.txt` lists `Sitemap: https://mentalvision.ai/sitemap.xml`.
